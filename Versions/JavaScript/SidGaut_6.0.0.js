...
function getChildEl(element, parent, index) {
	if (typeof element !== `string`)
		throw new Error(`First parameter must be an html element as a string, for getChildEl(Element, parent, index).`);
	else if (!(parent instanceof Element))
		throw new Error(`Second parameter must be a element for getChildEl(Element, parent, index).`);
	else if (typeof index !== `undefined` && typeof index !== `number`)
		throw new Error(`Third parameter must be a number for getEl(Element, index, index).`);
	else if (index === -1)
		...
		if (typeof index === `undefined`) {
			...
			throw new Error(`Third parameter must be a less than ${temp}, for getEl(Element, index, index).`);
		else {
			if (temp > 1)
				return parent.querySelectorAll(element)[index];
		}
		if (temp === 1)
...
Element.prototype.setVal = function(value) {
	this.value = `${value}`;
}
...
Node.prototype.delAttr = function(...attr) {
	let allValid = true;
	attr.forEach((e) => {
		if (typeof e !== `string`)
			allValid = false;
...
NodeList.prototype.delAttr = function(...attr) {
	let allValid = true;
	attr.forEach((e) => {
		if (typeof e !== `string`)
			allValid = false;
...
function dont(anything) {
	let imgLink  = `https://sidgautam.ca/js-lib/images/dont.png`;
	main.innerHTML = `<img src="${imgLink}" alt="Don't do that!" style="width: 100%; height: auto; display: block; margin: auto;">`;
}
